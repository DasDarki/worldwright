...
func RegisterRoutes(app *fiber.App, st *store.Store, authSvc *auth.Service, oauthSvc *auth.OAuth, adminSvc *admin.Service, assetsDir string) {
	...
	authed.Get("/recent", recentEntities(st))

	authed.Get("/entity-types", listEntityTypes(st))

	authed.Get("/entities", listEntities(st))
	authed.Post("/entities", requireAdmin, createEntity(st))
	authed.Post("/entities/reorder", requireAdmin, reorderEntities(st))
	authed.Get("/entities/by-slug/:slug", getEntityBySlug(st))
	authed.Patch("/entities/:id", requireAdmin, updateEntity(st))
	authed.Delete("/entities/:id", requireAdmin, deleteEntity(st))
	authed.Get("/entities/:id/backlinks", entityBacklinks(st))
	authed.Get("/entities/:id/relationships", listRelationships(st))
	authed.Get("/entities/:id/genealogy", entityGenealogy(st))

	authed.Get("/relationship-types", listRelationshipTypes(st))
	...
	authed.Get("/calendars", listCalendars(st))
	authed.Get("/calendars/:id", getCalendar(st))
	authed.Patch("/calendars/:id", requireAdmin, updateCalendar(st))

	authed.Get("/events", listEvents(st))
	authed.Get("/events/:id", getEvent(st))
	...
	authed.Get("/entities/:id/events", entityEvents(st))

	authed.Get("/timelines", listTimelines(st))
	authed.Get("/timelines/:id", getTimeline(st))
	...
	authed.Get("/assets/:id", serveAsset(st, assetsDir))
	authed.Delete("/assets/:id", requireAdmin, deleteAsset(st, assetsDir))

	authed.Get("/maps", listMaps(st))
	authed.Get("/maps/:id", getMap(st))

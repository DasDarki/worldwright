...
<template>
  <div v-if="editor" class="toolbar">
    <button type="button" class="t-btn" :class="{ on: is('bold') }" @mousedown.prevent @click="editor.chain().focus().toggleBold().run()" aria-label="Bold">
      <b>B</b>
    </button>
    <button type="button" class="t-btn italic" :class="{ on: is('italic') }" @mousedown.prevent @click="editor.chain().focus().toggleItalic().run()" aria-label="Italic">
      <i>I</i>
    </button>
    <span class="sep" />
    <select
    ...
    <span class="sep" />
    <button type="button" class="t-btn" @mousedown.prevent :class="{ on: is('blockquote') }" @click="editor.chain().focus().toggleBlockquote().run()" aria-label="Quote">&ldquo;</button>
    <button type="button" class="t-btn" @mousedown.prevent :class="{ on: is('bulletList') }" @click="editor.chain().focus().toggleBulletList().run()" aria-label="Bullet list">•</button>
    <button type="button" class="t-btn" @mousedown.prevent :class="{ on: is('orderedList') }" @click="editor.chain().focus().toggleOrderedList().run()" aria-label="Ordered list">1.</button>
    <span class="sep" />
    <button type="button" class="t-btn" @mousedown.prevent :class="{ on: is('spoiler') }" @click="editor.chain().focus().toggleSpoiler().run()" aria-label="Spoiler block">
    ...
    <button type="button" class="t-btn" @mousedown.prevent @click="emit('image')" aria-label="Insert image">
    ...
    <button type="button" class="t-btn wikilink" @mousedown.prevent @click="emit('wikilink')" aria-label="Insert wikilink">
      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round">
    ...
    <button type="button" class="t-btn" @mousedown.prevent :class="{ on: is('codeBlock') }" @click="toggleCodeBlock" aria-label="Code block">
      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
        <path d="M9 7 L4 12 L9 17 M15 7 L20 12 L15 17"/>
      </svg>
    </button>
    <button type="button" class="t-btn" @mousedown.prevent @click="insertTable" aria-label="Insert table">
    ...
    <button type="button" class="t-btn" @mousedown.prevent :class="{ on: is('callout') }" @click="toggleCallout" aria-label="Callout block">
      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
        <circle cx="12" cy="12" r="9"/>
        <path d="M12 8 V13 M12 16.5 V17"/>
      </svg>
    </button>
    <button type="button" class="t-btn graph" @mousedown.prevent @click="emit('graph')" aria-label="Relationship graph widget">
      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round">
    ...
    <button type="button" class="t-btn vault" @mousedown.prevent @click="emit('vault')" aria-label="Sealed section (author-only)">
      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">

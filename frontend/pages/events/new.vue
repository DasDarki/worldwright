...
const { t } = useI18n()
useHead({ title: () => t('events.newTitle') })
...
async function onSubmit(payload: any) {
  ...
  try {
    ...
    error.value = e?.data?.error || 'Failed to create event'
  } finally {
    pending.value = false
  }
}
...
<template>
  <section class="py-12 md:py-20">
    <div class="mx-auto max-w-screen-xl">
      ...
      <EventForm :submitting="pending" :submit-label="t('events.form.create')" @submit="onSubmit" />
    </div>
  </section>
</template>
